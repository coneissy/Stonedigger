...
async function dbRequest(body) {
  const response = await fetch(DB_URL, {
    ...
    headers: {
      ...
      "x-stonedigger-secret": DB_SECRET,
    },
...
bot.command("status", async (ctx) => {
  ...
  if (!user) {
    await syncUser(ctx);
    return ctx.reply("⛏️ Free account. Use /premium to unlock Premium for 100 ⭐.");
  }
...
bot.on("successful_payment", async (ctx) => {
  ...
  if (result.duplicate) {
    await ctx.reply("ℹ️ This payment was already recorded.\n⭐ Premium remains active on your account.");
    return;
  }
